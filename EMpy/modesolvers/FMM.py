...
class FMMMode2d(Mode):
    ...
    def TEfrac_old(self, x_=None, y_=None):
        ...
        cSTE = .5 * utils.trapz2(Ex * numpy.conj(cBy), y, x)
        cSTM = .5 * utils.trapz2(-Ey * numpy.conj(cBx), y, x)
    ...
    def overlap_old(self, m, x_=None, y_=None):
        ...
        norm = scipy.sqrt(utils.trapz2(cSz, y, x))
        Ex1, Ey1, Ez1, cBx1, cBy1, cBz1 = m.fields(x, y)
        cSz1 = m.intensity(x, y)
        norm1 = scipy.sqrt(utils.trapz2(cSz1, y, x))
        return .5 * utils.trapz2(Ex/norm * numpy.conj(cBy1/norm1) - Ey/norm * numpy.conj(cBx1/norm1), y, x)        
    ...
    def get_fields_for_FDTD(self, x, y):
        ...
        y_Ex_FDTD = y[1:-1]
        Ex_FDTD = utils.interp2(x_Ex_FDTD, y_Ex_FDTD, x0, y0, Ex)
        # Ey: ignores x = 0, max
        x_Ey_FDTD = x[1:-1]
        y_Ey_FDTD = utils.centered1d(y)
        Ey_FDTD = utils.interp2(x_Ey_FDTD, y_Ey_FDTD, x0, y0, Ey)
        # Ez: ignores x, y = 0, max
        x_Ez_FDTD = x[1:-1]
        y_Ez_FDTD = y[1:-1]
        Ez_FDTD = utils.interp2(x_Ez_FDTD, y_Ez_FDTD, x0, y0, Ez)
        # Hx: ignores x = 0, max, /120pi, reverse direction
        x_Hx_FDTD = x[1:-1]
        y_Hx_FDTD = utils.centered1d(y)
        Hx_FDTD = utils.interp2(x_Hx_FDTD, y_Hx_FDTD, x0, y0, cBx) / (-120. * numpy.pi) # OKKIO!
        # Hy: ignores y = 0, max, /120pi, reverse direction
        x_Hy_FDTD = utils.centered1d(x)
        y_Hy_FDTD = y[1:-1]
        Hy_FDTD = utils.interp2(x_Hy_FDTD, y_Hy_FDTD, x0, y0, Hy) / (-120. * numpy.pi)
        # Hz: /120pi, reverse direction
        x_Hz_FDTD = utils.centered1d(x)
        y_Hz_FDTD = utils.centered1d(y)
        Hz_FDTD = utils.interp2(x_Hz_FDTD, y_Hz_FDTD, x0, y0, Hz) / (-120. * numpy.pi)
