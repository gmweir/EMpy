...
import numpy
from . import constants   # EMpy.constants
from . import materials   # EMpy.materials
import scipy.linalg
...
class Layer(object):
    ...
    def getEPSFourierCoeffs(self, wl, n, anisotropic=True):
        ...
        if not anisotropic:
            ...
            EPS[:, :, hmax] = numpy.squeeze(
                self.mat.epsilonTensor(wl)) / constants.eps0
    ...
    def capacitance(self, area=1., wl=0):
        ...
        if self.isIsotropic():
            eps = constants.eps0 * numpy.real(self.mat.n(wl).item() ** 2)
        else:
...
class BinaryGrating(object):
    ...
    def getEPSFourierCoeffs(self, wl, n, anisotropic=True):
        ...
        if not anisotropic:
            ...
            EPS1 = numpy.zeros_like(EPS)
            eps1 = numpy.squeeze(
                self.mat1.epsilonTensor(wl)) / constants.eps0
            eps2 = numpy.squeeze(
                self.mat2.epsilonTensor(wl)) / constants.eps0
            f = self.dc
            h = numpy.arange(-hmax, hmax + 1)
    ...
    def getEPSFourierCoeffs(self, wl, n, anisotropic=True):
        ...
        if not anisotropic:
            ...
            eps2 = numpy.squeeze(
                self.mat2.epsilonTensor(wl)) / constants.eps0
            eps3 = numpy.squeeze(
                self.mat3.epsilonTensor(wl)) / constants.eps0
            f1 = self.dc1
            f2 = self.dc2
            h = numpy.arange(-hmax, hmax + 1)
            N = len(h)
...
class LiquidCrystalCell(object):
    ...
    def get_parameters(self, z=None):
        ...
        theta = rad2deg(numpy.pi / 2. - data[:, 0])
        phi = rad2deg(data[:, 2])
        u = data[:, 4]
    ...
    def createMultilayer(self):
        ...
        m = []
        for a, t in zip(deg2rad(self.angles), self.tlc):
            epsT = materials.EpsilonTensor(
                epsilon_tensor_const=euler_rotate(
                    numpy.diag([self.lc.nE,
                                self.lc.nO,
                                self.lc.nO]) ** 2,
                    0., numpy.pi / 2., numpy.pi / 2. - a) * constants.eps0,
                epsilon_tensor_known={
                    0: euler_rotate(
                        ...
                        0., numpy.pi / 2.,
                        numpy.pi / 2. - a) * constants.eps0,
                }
            )
            m.append(
                Layer(materials.AnisotropicMaterial(
...
def blackbody(f, T):
    return 2 * constants.h * f ** 3 / (constants.c ** 2) * 1. / (
        numpy.exp(constants.h * f / (constants.k * T)) - 1)
